...
DISCLAIMER = (
    "\n\n⚠️ **Disclaimer:** This bot does not host or promote any pirated content. "
    "All files are publicly available on the internet. The owner and developer of this bot "
    "are not responsible for any content shared through it."
)
...
async def cb_handler(client: Bot, query: CallbackQuery):
    ...
    if data == "help":
        ...
        await query.message.edit_text(
            # 👇 Append the disclaimer here
            text=ABOUT_TXT.format(first=query.from_user.first_name) + DISCLAIMER,
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("⚓ ʜᴏᴍᴇ", callback_data="start"),
